...
@retry(
    stop=stop_after_attempt(3), # 최대 3번 시
    before_sleep=lambda retry_state: logger.warning(
        f"API 호출 실패: {retry_state.outcome.exception()}, {retry_state.attempt_number}번째 시도 중..."
    )
)
...
async def main():
    ...
    # 비동기 함수 호출시 코루틴 객체 반환 (실행은 아직 안 됨)
