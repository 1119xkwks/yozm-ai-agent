...
random_generator = LlmAgent(
    ...
    instruction="스펨 메시지와 정상적인 메시지를 60:40 확률로 생성합니다. 스팸메시지는 '[웹발신]'을 앞에 넣고, 정상적인 메시지는 따로 표시하지 않아도 됩니다. 반드시 하나의 메시지만 출력해야합니다.",
)
...
spam_checker = LlmAgent(
    name="SpamChecker",
    model="gemini-2.5-flash",
    instruction="사용자의 입력이 스팸이지 확인하세요. 스팸이면 'fail', 아니면 'pass'를 반환하세요.",
