...
def get_coordinates(city_name: str) -> tuple[float, float]:
    ...
    geolocator = Nominatim(user_agent="whether_app")
    location = geolocator.geocode(city_name)
    if location:
        return location.latitude, location.longitude
    else:
        raise ValueError(f"Could not find coordicates for {city_name}")

def get_weather(city_name: str) -> dict:    # 날씩 관련 도구 함수
...
def get_kbo_rank() -> dict: # KBO 랭킹을 받아오는 도구 함수
    """한국 프로야구 구단의 랭킹을 가져오는 함수입니다."""
    response = httpx.get("https://sports.daum.net/prx/hermes/api/team/rank.json?leagueCode=kbo&sessionKey=2025")
    return response.json()
