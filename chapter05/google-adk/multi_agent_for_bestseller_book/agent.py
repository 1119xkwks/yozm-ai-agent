...
def get_book_search(search_keyword: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        """
        if search_keyword:
            result = httpx.get(f"http://api.itbook.store/1.0/search/{search_keyword}")
        else:
            result = httpx.get("http://api.itbook.store/1.0/new")
        return result.json()
        """
        # 대체
        ...
        params = {
            "q": search_keyword,
            "orderBy": "newest",  # 신간 느낌
            "maxResults": 10
        }
        res = httpx.get(url, params=params)
        return res.json()
    except Exception as e:
        return {"error": f"도서 정보를 가져오는데 실패했습니다: {str(e)}"}
